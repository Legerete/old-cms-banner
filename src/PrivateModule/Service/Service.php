...
namespace Wunderman\CMS\Banner\PrivateModule;
...
class Service implements ComposedPageExtension
{
	...
	public function addItem(Form $form)
	{
		...
		if ($form->isSubmitted()) {
			...
			if ($itemsContainer['add']->isSubmittedBy()) {
				$items++;
			}

			for ($i = 1; $i <= $items; $i++) {
				$container = $itemsContainer->addContainer($i);

				$container->addText('width', 'Width');
				$container->addText('height', 'Height');
				$container->addUpload('image', 'Image')
			...
			$form['items_count']->setValue($items);
		}
		...
		$item->addHidden('type')->setValue('Banner');
	}
